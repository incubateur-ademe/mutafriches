 ...
 * - 972090000O0498 : Martinique (972), commune 090, préfixe 000, section O, parcelle 0498
 *
...
export function isValidParcelId(id: string): boolean {
  ...
  // Pattern : [Dept 2-3 car][Commune 3 chiffres][Préfixe 0-3 car][Section 1-2 lettres][Parcelle 4 chiffres]
  const patterns = [
    // Départements métropole (01-95) + commune + préfixe optionnel + section + parcelle
    /^[0-9]{2}[0-9]{3}[0-9A-Z]{0,3}[A-Z]{1,2}[0-9]{4}$/,
    // Départements DOM (971-976) + commune + préfixe optionnel + section + parcelle
    /^97[1-6][0-9]{3}[0-9A-Z]{0,3}[A-Z]{1,2}[0-9]{4}$/,
    // Corse (2A, 2B) + commune + préfixe optionnel + section + parcelle
    /^2[AB][0-9]{3}[0-9A-Z]{0,3}[A-Z]{1,2}[0-9]{4}$/,
  ];
