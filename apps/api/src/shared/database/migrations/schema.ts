...
export const evenementsUtilisateur = pgTable("evenements_utilisateur", {
	id: varchar({ length: 50 }).primaryKey().notNull(),
	...
	index("idx_date_creation").using("btree", table.dateCreation.asc().nullsLast().op("timestamp_ops")),
	index("idx_evaluation_id").using("btree", table.evaluationId.asc().nullsLast().op("text_ops")),
	index("idx_type_evenement").using("btree", table.typeEvenement.asc().nullsLast().op("text_ops")),
]);

export const logsEnrichissement = pgTable("logs_enrichissement", {
	id: varchar({ length: 50 }).primaryKey().notNull(),
	...
	index("idx_logs_enrichissement_centroid").using("btree", table.centroidLatitude.asc().nullsLast().op("numeric_ops"), table.centroidLongitude.asc().nullsLast().op("numeric_ops")),
	index("idx_logs_enrichissement_date").using("btree", table.dateEnrichissement.asc().nullsLast().op("timestamp_ops")),
	index("idx_logs_enrichissement_identifiant").using("btree", table.identifiantCadastral.asc().nullsLast().op("text_ops")),
	index("idx_logs_enrichissement_statut").using("btree", table.statut.asc().nullsLast().op("text_ops")),
]);
