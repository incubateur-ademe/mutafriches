...
export class UiService {
  ...
  private loadLayout(layoutName: string): string {
    const layoutPath = join(this.getBasePath(), "layouts", `${layoutName}.html`);
    return readFileSync(layoutPath, "utf8");
  }

  private loadComponent(componentName: string): string {
    const componentPath = join(this.getBasePath(), "components", `${componentName}.html`);
  ...
  renderBasePage(title: string, components: Component[], pages: Page[]): string {
  ...
  renderFormStep(
    ...
    if (stepNumber < 1 || stepNumber > 3) {
      throw new Error(`Invalid step number: ${stepNumber}. Must be between 1 and 3.`);
    }
    ...
    const components = [
      {
        ...
        data: {
          ...
          nextStepTitle: validStepNumber < 3 ? config.nextTitle : "Analyse terminée",
    ...
    return this.renderBasePage(`Mutafriches - ${config.title}`, components, pages);
  }
  ...
  private extractSessionDisplayData(sessionData?: FormSessionData | null): {
    ...
    Object.entries(sessionData.parcelleData).forEach(([key, dataWithSource]) => {
      if (dataWithSource.value !== undefined && dataWithSource.value !== null) {
        // Formatage spécifique selon le type de donnée
        displayData[key] = this.formatValueForDisplay(key, dataWithSource.value);
   ...
   */
  transformEnrichmentForUI(enrichmentResult: EnrichmentResultDto): UiParcelleDto {
    return {
      // Données de base
      ...
      identifiantParcelle: enrichmentResult.identifiantParcelle || "Non renseigné",
      connectionElectricite: enrichmentResult.connectionReseauElectricite ? "Oui" : "Non",
      distanceRaccordement: this.formatDistance(
        enrichmentResult.distanceRaccordementElectrique,
        "m",
      ),
      risquesNaturels: enrichmentResult.presenceRisquesNaturels?.toString() || "Non renseigné",
   ...
   */
  transformMutabilityForUI(mutabilityResult: MutabilityResultDto): UiMutabilityResultDto {
    return {
      fiabilite: {
