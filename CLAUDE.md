...
pnpm db:zones-contrainte-enr:import <capca.json>  # Importer les zones de contrainte réseau EnR (Enedis, fichier manuel)
```
...
- **Référentiel local Zones de contrainte EnR** (`raw_zones_contrainte_enr`) : zones de postes sources saturées pour raccorder des projets EnR (Enedis/RTE), importées chaque mois depuis un fichier **téléchargé à la main** (protection anti-robots, pas d'API) — rappel par issue GitHub mensuelle (ADR-0046)
